...
if __name__ == '__main__':
    ...
    parser.add_argument('--bw', type=int, default=0, help='bit width of quantization')
    ...
    tools.add_arguments_for_module(parser, datasets, argument_for_class='training_dataset2', default='MpiSintelFinal', 
                                    skip_params=['is_cropped'],
                                    parameter_defaults={'root': './MPI-Sintel/flow/training'})
    ...
    tools.add_arguments_for_module(parser, datasets, argument_for_class='validation_dataset2', default='MpiSintelClean', 
                                    skip_params=['is_cropped'],
                                    parameter_defaults={'root': './MPI-Sintel/flow/training',
                                                        'replicates': 1})
    ...
    with tools.TimerBlock("Parsing Arguments") as block:
        ...
        args.training_dataset2_class = tools.module_to_dict(datasets)[args.training_dataset2]
        args.validation_dataset_class = tools.module_to_dict(datasets)[args.validation_dataset]
        args.validation_dataset2_class = tools.module_to_dict(datasets)[args.validation_dataset2]
    ...
    with tools.TimerBlock("Initializing Datasets") as block:
        ...
        if exists(args.training_dataset2_root):
            train_dataset2 = args.training_dataset2_class(args, True, **tools.kwargs_from_args(args, 'training_dataset2'))
            block.log('Training Dataset 2: {}'.format(args.training_dataset2))
            block.log('Training Input 2: {}'.format(' '.join([str([d for d in x.size()]) for x in train_dataset2[0][0]])))
            block.log('Training Targets 2: {}'.format(' '.join([str([d for d in x.size()]) for x in train_dataset2[0][1]])))
            train_loader2 = DataLoader(train_dataset2, batch_size=args.effective_batch_size, shuffle=True, **gpuargs)
        ...
        if exists(args.validation_dataset2_root):
            validation_dataset2 = args.validation_dataset2_class(args, True, **tools.kwargs_from_args(args, 'validation_dataset2'))
            block.log('Validation Dataset 2: {}'.format(args.validation_dataset2))
            block.log('Validation Input 2: {}'.format(' '.join([str([d for d in x.size()]) for x in validation_dataset2[0][0]])))
            block.log('Validation Targets 2: {}'.format(' '.join([str([d for d in x.size()]) for x in validation_dataset2[0][1]])))
            validation_loader2 = DataLoader(validation_dataset2, batch_size=args.effective_batch_size, shuffle=False, **gpuargs)
    ...
    with tools.TimerBlock("Building {} model".format(args.model)) as block:
        ...
        train_logger2 = SummaryWriter(log_dir = os.path.join(args.save, 'train'), comment = 'training2')
        validation_logger = SummaryWriter(log_dir = os.path.join(args.save, 'validation'), comment = 'validation')
        validation_logger2 = SummaryWriter(log_dir = os.path.join(args.save, 'validation'), comment = 'validation2')
    ...
    def train(args, epoch, start_iteration, data_loader, model, optimizer, logger, is_validate=False, offset=0):
        ...
        for batch_idx, (data, target) in enumerate(progress):
            ...
            # process the weights including quantization
            if args.bw != 0:
                qo.quantization()
            ...
            v_statistics.append(loss_values)
            t_statistics.append(loss_values)
            if is_validate:
            ...
            progress.set_description(title + ' ' + tools.format_dictionary_of_losses(loss_labels, np.array(statistics).mean(axis=0)))

            if ((((global_iteration + 1) % args.log_frequency) == 0 and not is_validate) or
                #(is_validate and batch_idx == args.validation_n_batches - 1)):
                (is_validate and batch_idx == len(data_loader) - 1)):
    ...
    def inference(args, epoch, data_loader, model, offset=0):
        ...
        progress = tqdm(data_loader, ncols=120, total=np.minimum(len(data_loader), args.inference_n_batches), desc='Inferencing ', 
    ...
    best_err = 1e8
    best_err2 = 1e8
    progress = tqdm(list(range(args.start_epoch, args.total_epochs + 1)), miniters=1, ncols=120, desc='Overall Progress', leave=True, position=0)
    offset = 1
    ...
    for epoch in progress:
        ...
        if not args.skip_validation and ((epoch - 1) % args.validation_frequency) == 0:
            ...
            #offset += 1
            if exists(args.validation_dataset2_root):
                validation_loss2, _ = train(args=args, epoch=epoch - 1, start_iteration=global_iteration, data_loader=validation_loader2, model=model_and_loss, optimizer=optimizer, logger=validation_logger2, is_validate=True, offset=offset)
                total_loss += validation_loss2
                #offset += 1
            ...
            if total_loss < best_err:
                ...
                if exists(args.validation_dataset2_root):
                    best_err2 = validation_loss2
                is_best = True

            checkpoint_progress = tqdm(ncols=120, desc='Saving Checkpoint', position=offset)
            if exists(args.validation_dataset2_root):
        ...
        if not args.skip_training:
            ...
            #offset += 1
            if exists(args.training_dataset2_root):
                train_loss2, iterations = train(args=args, epoch=epoch, start_iteration=global_iteration, data_loader=train_loader2, model=model_and_loss, optimizer=optimizer, logger=train_logger2, offset=offset)
                #offset += 1
